...
class CNNModel:
    ...
    def __init__(self, height, width, channels, classes, learning_rate, mean, std):
        ...
        self.conv1  = tf.keras.layers.Conv2D(32, [3,3], strides=(2,2), padding='same', activation=tf.nn.relu, name='conv_1')(self.x)
        self.drop1  = tf.keras.layers.Dropout(0.25, name='dropout_cv1')(self.conv1, training=self.is_training )
        ...
        self.conv2  = tf.keras.layers.Conv2D(64, [3,3], strides=(2,2), padding='same', activation=tf.nn.relu, name='conv_2')(self.norm1)
        self.drop2  = tf.keras.layers.Dropout(0.25, name='dropout_cv2')(self.conv2, training=self.is_training )
        ...
        self.conv3  = tf.keras.layers.Conv2D(64, [3,3], strides=(2,2), padding='same', activation=tf.nn.relu, name='conv_3')(self.norm2)
        self.drop3  = tf.keras.layers.Dropout(0.25, name='dropout_cv3')(self.conv3, training=self.is_training )
        print('convolution_3_dim: ', self.conv3.shape)
        self.flat   = tf.keras.layers.Flatten()(self.drop3)
        print('flat_dim: ', self.flat.shape)
        self.dense1 = tf.keras.layers.Dense(64, name='dense')(self.flat)
        print('dense_1_dim: ', self.dense1.shape)
        self.drop4  = tf.keras.layers.Dropout(0.5, name='dropout')(self.dense1, training=self.is_training )
